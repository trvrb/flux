...
In the case of HI, the measurement of cross-reactivity takes the form of a titer representing the dilution factor at which serum raised against a particular virus ceases to be effective at inhibiting binding of another virus.  
...
Let $\virus_i \in \mathbb{R}^{P}$ represent the cartographic location of virus $i$ for $i = 1,\ldots,\vn$, so that $\virus_i = (x_{i1}, x_{i2})^{\prime}
%\twomatrix{x_{i1}}{x_{i2}}
$ for $P=2$. 
Similarly, let $\serum_j$ represent the cartographic location of serum $j$ for $j = 1,\ldots,\sn$, so that $\serum_j = (y_{j1},y_{j2})^{\prime}
%\twomatrix{y_{j1}}{y_{j2}}
$ for $P=2$.
...
%This yields an $\vn \times P$ matrix $\viruses$ of virus coordinates and an $\sn \times P$ matrix $\sera$ of serum coordinates.
...
\begin{equation} \label{hij}
	H_{ij} \sim \mbox{Normal}( \se_j - \delta_{ij}, \mdssd^2 ).
\end{equation}
Consequentially, the likelihood of observing an exact titer given the placement of antigenic locations is 
\begin{equation} 
	\point(H_{ij}) = \phi \left( \frac{ H_{ij} + \delta_{ij} - \se_j }{ \mdssd } \right),
\end{equation}
where 
%$\mdssd^2$ represents variance and 
$\phi(\cdot)$ represents the standard normal probability density function (PDF).
%The `serum effect' $\se_j$ represents the expected log$_2$ titer between virus $i$ and serum $j$ when $\delta_{ij}=0$. and is initially fixed such that $\se_j = \max ( H_{1j},\ldots,H_{\vn j} )$. %% MAS: Repeated material from above.
...
In the antigenic setting, these remain random and can be negative (I still don't really believe this) since neither $\se_j$ is known nor is $H_{ij}$ observed with much precision. 
...
\end{equation}
where $\Phi(\cdot)$ represents the standard normal cummulative distribution fucntion (CDF).
...
We assume that the prior precision $1/\mdssd^2$ follows a diffuse $\mbox{Gamma}(a, b)$ distribution with $a=0.001$ and $b=0.001$, and we start by assuming independent, diffuse normal prior on virus and serum locations
\begin{eqnarray}
	\virus_i &\sim& \mathrm{Normal}(\boldsymbol{\mu},\boldsymbol{\Sigma}) \nonumber \\
	\serum_j &\sim& \mathrm{Normal}(\boldsymbol{\mu},\boldsymbol{\Sigma}),
\end{eqnarray}
with $\boldsymbol{\mu} = (0,\ldots,0)^{\prime} 
%\twomatrix{0}{0}
$ and $\boldsymbol{\Sigma}
%\fourmatrix{10,000}{0}{0}{10,000}
$ is a diagonal matrix with diagonal elements all equalling $10000$.
%for $P=2$.
...
Metropolis-Hastings proposals include transition kernels for individual virus and serum locations $\virus_i$ and $\serum_j$, scaling of the entire set of virus and serum locations $\viruses$ and $\sera$ and scaling $\mdssd^2$.
...
We take an Empirical Bayesian approach is specifying the 
mean and variance of this distribution, set to the empirical mean and empirical variance of the set of maximum titers across sera $\{ \max ( H_{1j},\ldots,H_{\vn j} ) : j = 1,\ldots,\sn \}$.
This formulation assumes that particular sera are more reactive in general than other sera.

Additionally, we follow the same logic and assume that some virus isolates are more reactive in general than other virus isolates.
With virus reactivity included, the expectation of $H_{ij}$ becomes $(\ve_i+\se_j)/2 - \delta_{ij}$ and the vector of `virus effects' $\ve_i$ for $i = 1,\ldots, \vn$ is estimated in an analogous hierarchical fashion, with $\ves$ normally distributed with mean and variance equal to the empirical mean and variance of the set of maximum titers across viruses $\{ \max ( H_{i1},\ldots,H_{i \sn} ) : i = 1,\ldots,\vn \}$.

With these effects included, Metropolis-Hastings proposals additionally incorporate transition kernels for individual serum effects $\se_j$ and virus effects $\ve_i$. \textbf{What are $\ve_i$?  These have not been defined nor used.}

\subsection*{Antigenic drift prior}

As presented, virus and serum locations $\viruses$ and $\sera$ accurately characterize antigenic distances between virus strains based on titers from pairwise HI assays.
...
To capture this, we replace the diffuse normal prior on virus locations with a prior where virus locations follow a normal distribution in which the expectation of dimension 1 increases with date of virus sampling
\begin{equation}
	\virus_i \sim \mathrm{Normal}(\mu, \Sigma),
\end{equation}
where $\mu = \twomatrix{\beta \, t_i}{0}$ and $\Sigma = \fourmatrix{\driftsd^2}{0}{0}{\driftsd^2}$ for $P=2$, and $t_i$ is the difference in time between virus $i$ and the earliest sampled virus.
...
\begin{equation}
	(\virus_1,\ldots,\virus_n) \sim \mbox{Evolutionary Brownian Process}(\tree, \diffusionsd),
\end{equation}
...
The probability of observing virus locations $p(\viruses | \tree, \diffusionsd)$ is determined through analytical integration across internal states following the methods introduced in \cite{Lemey10}.
...
\begin{equation}
	p(\viruses,\sera | \mathbf{H},\tree) = \frac{ p(\mathbf{H} | \viruses, \sera, \ses, \ves, \mdssd) \; 
	p(\viruses | \beta, \driftsd) \;
	p(\viruses | \tree, \diffusionsd) \; 
	p(\sera, \ses, \ves, \mdssd, \beta, \driftsd, \diffusionsd)}{ p(\mathbf{H}) }.
\end{equation}
...
Our `antigenic drift prior' removes much of this issue, orienting the antigenic map along dimension 1 and fixing it to begin at the origin.
...
In this case, $p(\mathbf{H}|\viruses,\sera,\ses,\ves, \mdssd)$ is reduced, but $p(\viruses | \tree, \diffusionsd)$ becomes very large, yielding a greater overall posterior probability.
...
MCMC was used to sample virus locations $\viruses$, serum locations $\sera$, virus effects $\ves$, serum effects $\ses$, MDS precision $1/\mdssd^2$, antigenic drift rate $\beta$, population variance $\driftsd^2$, diffusion volatility $\diffusionsd$ and phylogenetic tree $\tree$.
