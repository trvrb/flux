...
Our goal is to find an optimal projection of the high-dimensional distance matrix $\mathbf{H}$ into a lower number of dimensions $P$. 
...
Let $\virus_i \in \Re^{P}$ represent the cartographic location of virus $i$ for $i = 1,\ldots,\vn$, so that $\virus_i = \twomatrix{x_{i1}}{x_{i2}}$ for $P=2$. 
Similarly, let $\serum_j$ represent the cartographic location of serum $j$ for $j = 1,\ldots,\sn$, so that $\serum_j = \twomatrix{y_{j1}}{y_{j2}}$ for $P=2$.
...
\begin{equation}
	d_{ij} =  \se_j - H_{ij},
\end{equation}
where $H_{ij}$ is the log$_2$ titer of virus $i$ against serum $j$ and $\se_j = \max ( H_{1j},\ldots,H_{\vn j} )$.
In following multidimensional scaling (MDS), these approaches attempt to optimize over $\viruses$ and $\sera$ such that
\begin{equation}
	\sum_{(i,j) \in \cal I} 
	\left(
		\delta_{ij} - d_{ij}
...
\end{equation}
and the likelihood of observing an exact titer given the placement of antigenic locations is 
\begin{equation} 
...
The `serum effect' $\se_j$ represents the expected log$_2$ titer between virus $i$ and serum $j$ when $\delta_{ij}=0$ and is initially fixed such that $\se_j = \max ( H_{1j},\ldots,H_{\vn j} )$.
...
We assume that the prior precision $1/\mdssd^2$ follows a diffuse $\mbox{Gamma}(a, b)$ distribution with $a=0.001$ and $b=0.001$, and we start by assuming a diffuse normal prior on virus and serum locations
\begin{eqnarray}
	\virus_i &\sim& \mathrm{Normal}(\mu,\Sigma) \nonumber \\
	\serum_j &\sim& \mathrm{Normal}(\mu,\Sigma),
\end{eqnarray}
with $\mu = \twomatrix{0}{0}$ and $\Sigma = \fourmatrix{10,000}{0}{0}{10,000}$ for $P=2$.
...
Metropolis-Hastings proposals include moves to individual virus and serum locations $\virus_i$ and $\serum_j$, scaling of the entire set of virus and serum locations $\viruses$ and $\sera$ and scaling $\mdssd$.
...
However, this model may biased in some circumstances.
...
In this case, $H_{ij}$ still follows equation \ref{hij} with expectation $\se_j - \delta_{ij}$, but the vector of `serum effects' $\ses$ is estimated rather than fixed.
...
With these effects included, Metropolis-Hastings proposals additionally incorporate moves to individual serum effects $\se_j$ and virus effects $\ve_i$.
