...
% Antigenic drift determines evolutionary success of seasonal influenza lineages
...
In the antigenic setting, these remain random and can be negative \msc{I still don't really believe this} since neither $\se_j$ is known nor is $H_{ij}$ observed with much precision. 
...
\end{equation}
where $\Phi(\cdot)$ represents the standard normal cumulative distribution fucntion (CDF).
...
We take an Empirical Bayesian approach is specifying the mean and variance of this distribution, set to the empirical mean and empirical variance of the set of maximum titers across sera $\{ \max ( H_{1j},\ldots,H_{\vn j} ) : j = 1,\ldots,\sn \}$.
...
With these effects included, Metropolis-Hastings proposals additionally incorporate transition kernels for individual serum effects $\se_j$ and virus effects $\ve_i$. \msc{What are $\ve_i$?  These have not been defined nor used.}
