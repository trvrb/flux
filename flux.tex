...
However, we observe occassional large jumps in antigenic phenotype (Figure~\ref{drift}), corresponding to cluster transitions identified by Smith et al. \cite{Smith04}.  
...
Interestingly, although antigenic clusters seem to emerge in a punctuated fashion in influenza A/H3N2, year-to-year antigenic drift remains relatively constant through time (Figure~\ref{jumps}).
For example, the largest cluster transition we observe is the emergence of the Sydney/97 cluster in 1997, evident as a large gap in virus locations along dimension 1 in Figure~\ref{drift}.
...
In 1999, the transition is complete.
...
A similar pattern is evident in the Fujian/02 transition (Figure~\ref{jumps}).
Although A/H3N2 shows significantly greater mean rates of drift than A/H1N1 and influenza B, year-to-year variation in antigenic drift is fairly similar across influenza clades.
We find that the standard deviation across years in year-to-year drift is 0.74 antigenic units for A/H3N2, 0.56 units for A/H1N1, 0.72 units for B/Vic and 0.38 units for B/Yam.

%%% jumps %%%
\begin{figure}[h]
	\centering		
	\includegraphics[width=0.5\textwidth]{figures/jumps}
	\caption{\textbf{Year-to-year antigenic drift of A/H3N2, A/H1N1, B/Vic and B/Yam viruses.} 
	Year-to-year antigenic drift for a virus clade for year $i$ is measured as the mean of antigenic dimension 1 of viruses collected in year $i$ compared to the mean of antigenic dimension 1 of viruses collected during the previous year $i-1$.
	Bars represent interquartile range across MCMC samples.
	} 
	\label{jumps} 
\end{figure}
...
To rule out effects caused by the 2009 pandemic, incidence data were examined only until 2008.
From 1999 to 2008, A/H3N2 accounted for the majority of antigenic drift (51\%), while A/H1N1, B/Vic and B/Yam split the remainder, accounting for 22\%, 15\% and 13\%, respectively.
...
These proportions are significantly similar to one another (randomization test, $p = 0.001$).
...
\begin{figure}[tb]
	...
	Drift for a virus clade for year $i$ is measured as the mean of antigenic dimension 1 of viruses collected in year $i$ compared to the mean of antigenic dimension 1 of viruses collected during the previous year $i-1$.
	...
	Years lacking antigenic information (Figure~\ref{jumps}) were not included in the correlation.
	} 
	\label{incidence} 
\end{figure}
...
Here, we find an overall correlation between clade-specific relative antigenic drift and clade-specific incidence of $r = 0.74$ (Pearson correlation, $p = 2.43 \times 10^{-7}$).
Correlation is present to greater of lesser degrees for each of the four clades, with $r_\mathrm{H3} = 0.61$, $r_\mathrm{H1} = 0.68$, $r_\mathrm{Vic} = 0.23$ and $r_\mathrm{Yam} = 0.56$.
