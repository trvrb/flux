...
With these effects included, Metropolis-Hastings proposals additionally incorporate transition kernels for individual serum effects $\se_j$ and virus effects $\ve_i$.
...
\begin{equation}
	\virus_i \sim \mathrm{Normal}(\boldsymbol{\mu}, \boldsymbol{\Sigma}),
\end{equation}
where $\boldsymbol{\mu} = (\beta \, t_i, 0)^{\prime}$ and $\boldsymbol{\Sigma} = \fourmatrix{\driftsd^2}{0}{0}{\driftsd^2}$ for $P=2$, and $t_i$ is the difference in time between virus $i$ and the earliest sampled virus.
