...
We begin with a BMDS analog of the model used by Smith et al.\ \cite{Smith04}, in which viruses and sera are represented as 2D locations, expected titer is relative to the maximum titer of a particular ferret serum and virus and serum locations follow a flat prior.
...
We extend this model to include a prior on virus and serum locations that assumes antigenic location along dimension 1 increases in proportion to date of virus or serum sampling, resulting in a small improvement in predictive error (model 2; Table \ref{errortable}).
...
\begin{table}[h]
	...
	\hline
	\multicolumn{4}{c}{} 	&	\multicolumn{4}{c}{Test error}  \\
	Model	&	Dimen 	& 	Serum/virus effects 	& Location Prior					& 	A/H3N2	& 	A/H1N1	& 	B/Vic	& 	B/Yam	\\
	\hline		
	1		&	2D 		&	Fixed $\ses$ 			& Uninformed						&	0.93 	&	0.78 	&	0.80	&	0.93	\\
	2		&	2D 		&	Fixed $\ses$			& $\beta,\virussd,\serumsd$			&	0.91 	&	0.74 	&	0.77	&	0.83	\\	
	3		&	1D 		&	Fixed $\ses$			& $\beta,\virussd,\serumsd$			&	1.07	&	0.92	&	0.94	&	1.08	\\		
	4		&	3D 		&	Fixed $\ses$			& $\beta,\virussd,\serumsd$			&	0.91	&	0.74 	&	0.79	&	0.84	\\
	5		&	4D 		&	Fixed $\ses$			& $\beta,\virussd,\serumsd$			&	0.95	&	0.75 	&	0.81	&	0.86	\\
	6		&	5D 		&	Fixed $\ses$			& $\beta,\virussd,\serumsd$			&	0.99	&	0.76 	&	0.84	&	0.88	\\				
	7		&	2D 		&	Estimated $\ses, \ves$	& $\beta,\virussd,\serumsd$			&	0.77	&	0.71 	&	0.65	&	0.73	\\	
	8		&	2D 		&	Estimated $\ses, \ves$	& $\beta,\virussd,\serumsd,\tree$	&	0.76	&	0.71 	&	0.64	&	0.72	\\		
	\hline
	\end{tabular}	
	}
\end{table}
...
Increasing dimensionality further to four or five dimensions results in continued declines in test error (models 5 and 6; Table \ref{errortable}).
...
We include sequence data for A/H3N2, A/H1N1, B/Vic and B/Yam to estimate this diffusion process, and find a small increase in predictive accuracy when including covariance induced by the diffusion (model 8; Table \ref{errortable}).
...
Interestingly, a minor lineage of B/Vic has persisted, appearing antigenically distinct from 2011 viruses (Figure~\ref{drift}), though the eventual fate of this lineage has yet to be determined.
Influenza B/Yamagata evolved slower still, with an average rate of and 0.21 (HPD 0.10--0.28) units per year.
Little punctuated evolution is apparent in the evolution of B/Yam.
...
We compare these measurements of relative antigenic drift to relative isolations of viruses from each clade in the USA from 1998 to 2011 (see Methods).
From 1998 to 2011, A/H3N2 accounted for the majority of antigenic drift (56\%), while A/H1N1, B/Vic and B/Yam split the remainder, accounting for 15\%, 21\% and 7\%, respectively.
