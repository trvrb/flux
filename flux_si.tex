...
\begin{eqnarray}
	d_\mathrm{total} = \frac{1}{n} \sum^n_{i=1} d_i^2 \nonumber \\
	t_\mathrm{total} = \frac{1}{n} \sum^n_{i=1} t_i^2 \nonumber \\
	\hat{D} = \frac{ d_\mathrm{total} }{ 4 t_\mathrm{total} }.
\end{eqnarray}
...
When estimating $D$ specific to trunk branches or side branches, we calculate $d_\mathrm{total}$ and $t_\mathrm{total}$ only from the subset of trunk branches or side branches as appropriate.
