...
In the course of the MCMC, we sample tip locations and internal node locations. [THIS IS NOT TRUE]
At each MCMC step, we conduct a linear interpolation between these sampled locations to arrive at $x$ and $y$ values at intervals of 0.05 years for each virus. [THIS IS NOT TRUE]
Then, these $x$ and $y$ values are analyzed across MCMC samples at each time point to arrive at a mean posterior location. [analyzed -> summarized, location -> location estimate]
We draw lines between these locations to show mean posterior diffusion paths. [show -> approximate?]
